...
/**
 * Script to remove "eclipse" topic from specific repositories where it was just added
 * This is a one-time undo script for the accidental eclipse topic addition
 */
...
// Repositories where eclipse was just added (from the logs)
// Updated list includes repos from both accidental additions
const REPOS_TO_FIX = [
  'odm-ondocker',
 ...
 */
async function undoEclipseAddition() {
  console.log('🔄 Undoing accidental "eclipse" topic addition...\n');
  ...
  for (const repoName of REPOS_TO_FIX) {
    ...
    try {
      ...
      if (!currentTopics.includes('eclipse')) {
        console.log(`[${processedCount}/${REPOS_TO_FIX.length}] ⏭️  ${repoName} - eclipse not found, skipping`);
        skippedCount++;
        continue;
      }

      const newTopics = currentTopics.filter(topic => topic !== 'eclipse');
      
      console.log(`\n[${processedCount}/${REPOS_TO_FIX.length}] 📁 ${repoName}`);
      console.log(`📋 Current topics: ${currentTopics.join(', ')}`);
      console.log(`🗑️  Removing: eclipse`);
      console.log(`🎯 New topics: ${newTopics.length > 0 ? newTopics.join(', ') : '(none)'}`);

      if (isDryRun) {
        console.log('🔍 DRY RUN: Would remove eclipse');
      } else {
        if (!GITHUB_TOKEN) {
        ...
        try {
          ...
          console.log('✅ Eclipse removed successfully');
          updatedCount++;
  ...
  console.log(`   Skipped (no eclipse): ${skippedCount} repositories`);
  if (!isDryRun) {
...
// Run the script
undoEclipseAddition().catch(error => {
